...
The mapped entity still needs `#[canyon_entity]` and `CanyonMapper`:
...
Each operation has a `_with` version for a named datasource or compatible connection. `EntityCrud` is the composite runtime contract; it is implemented when insert, update, and delete are available for the same mapped entity. `Read` can be derived separately on a repository adapter to provide `find_all`, `find_by_pk`, `count`, and a select builder returning the mapped type.
