...
Canyon maps existing tables; the ordinary CRUD path does not provision your schema. The PostgreSQL `teams` table in the [installation example](./initial_setup.md) has an identity key and a non-null name, matching the model in the next chapter.

If your table uses different names, tell Canyon with `#[canyon_entity(table_name = "...", schema = "...")]`.

## Canyon's test databases
